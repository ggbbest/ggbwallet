...
import static com.ggbwallet.app.entity.MagicLinkParcel.generateReverseTradeData;
import static com.ggbwallet.token.tools.ParseMagicLink.currencyLink;
import static com.ggbwallet.token.tools.ParseMagicLink.customizable;
...
import static com.ggbwallet.token.tools.ParseMagicLink.unassigned;
...
public class ImportTokenViewModel extends BaseViewModel
{
    ...
    public void importThroughFeemaster(String url)
    {
        try
        {
            initParser();
            MagicLinkData order = parser.parseUniversalLink(universalImportLink);
            disposable = ggbWalletService.handleFeemasterImport(url, wallet.getValue(), network.getValue().chainId, order)
                    .subscribeOn(Schedulers.io())
                    .observeOn(AndroidSchedulers.mainThread())
    ...
    {
        disposable = ggbWalletService.checkFeemasterService(feemasterServer, network.getValue().chainId, importOrder.contractAddress)
                .subscribeOn(Schedulers.io())
                .observeOn(AndroidSchedulers.mainThread())
